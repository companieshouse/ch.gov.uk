package ChGovUk::Controllers::Company::LimitedPartners;
...
sub list {
    ...
    trace "Get !LIMITED PARTNERS! company officer list for %s, page %s", $company_number, $page [OFFICER LIST];
    ...
    $self->ch_api->company($company_number)->officers({
        ...
        filter         => $filter,
        register_type  => 'limited_partners',
        register_view  => 'true',
    })->get->on(
        success => sub {
            my ($api, $tx) = @_;
