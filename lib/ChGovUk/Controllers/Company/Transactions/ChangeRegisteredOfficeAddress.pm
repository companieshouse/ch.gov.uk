...
sub get {
    ...
    $self->app->log->debug("In forms-ad01#get [GET]");
    ...
    $self->ch_api->transactions($self->stash('transaction_number'))->registered_office_address->get->on(
        failure => sub {
            ...
            if ($error_code == 401) {
                ...
                $self->app->log->warn("Company (" . $self->stash('company_number') . ") unauthorised. Redirecting to company login, with return_to: $return_to [ROUTING]");
        ...
        success => sub {
            my ($api, $tx) = @_;
            my $addr = ChGovUk::Models::Address->new->from_api_hash($tx->res->json);
