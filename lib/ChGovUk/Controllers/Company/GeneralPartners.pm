package ChGovUk::Controllers::Company::GeneralPartners;
...
sub list {
    ...
    trace "Get !GENERAL PARTNERS! company officer list for %s, page %s", $company_number, $page [OFFICER LIST];
    ...
    $self->ch_api->company($company_number)->officers({
        ...
        filter         => $filter,
        register_type  => 'general_partners',
        register_view  => 'true',
    })->get->on(
        success => sub {
            my ($api, $tx) = @_;
...
sub list {
    ...
    $self->stash( generalPartners => $model, );
